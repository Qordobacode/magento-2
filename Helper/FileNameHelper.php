 ...
 */
class FileNameHelper extends \Magento\Framework\App\Helper\AbstractHelper implements
    \Qordoba\Connector\Api\Helper\FileNameHelperInterface
{
    /**
     * @param string $name
    ...
    public function getFileName($name, $nameSeparator = self::FILE_NAME_SEPARATOR)
    {
        ...
        $fileName = preg_replace('/[\\/_|+ -]+/', $nameSeparator, $fileName);
        return $fileName;
    }
    ...
    public function getFileNameByModel(\Magento\Framework\Model\AbstractModel $model)
    {
        ...
        if ($model instanceof \Magento\Catalog\Api\Data\CategoryInterface) {
            $fileName = sprintf(
                'catalog-product-category-%s-%s',
                $model->getId(),
                $this->getFileName($model->getName())
            );
        } elseif ($model instanceof \Magento\Cms\Api\Data\BlockInterface) {
            $fileName = sprintf('cms-block-%s-%s', $model->getId(),$this->getFileName($model->getTitle()));
        } elseif ($model instanceof \Magento\Cms\Api\Data\PageInterface) {
            $fileName = sprintf('cms-page-%s-%s', $model->getId(),$this->getFileName($model->getTitle()));
        } elseif ($model instanceof \Magento\Catalog\Api\Data\ProductInterface) {
            $fileName = sprintf('catalog-product-%s-%s', $model->getId(),$this->getFileName($model->getName()));
        } elseif ($model instanceof \Magento\Eav\Api\Data\AttributeInterface) {
            $fileName = sprintf(
                'catalog-product-attribute-%s-%s',
                $model->getId(),
                $this->getFileName($model->getDefaultFrontendLabel())
            );
        }
        return $fileName;
    }
